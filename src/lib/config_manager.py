...
class ConfigManager:
    ...
    def _get_config_path(self) -> Path:
        ...
        if system == "Windows":
            ...
            home = Path.home()
            config_dir = home / 'Library' / 'Application Support' / 'video-duplicate-scanner'
        else:
    ...
    def add_scan_history(self, directory_path: str, file_count: int, duplicates_found: int) -> None:
        ...
        scan_entry = {
            'path': abs_path,
            'last_scanned': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z'),
        ...
        scanned_dirs = [entry for entry in scanned_dirs if entry.get('path') != abs_path]
        ...
        config['total_files_processed'] = config.get('total_files_processed', 0) + file_count
        config['total_duplicates_found'] = config.get('total_duplicates_found', 0) + duplicates_found
