...
def show(output_format: str):
    ...
    try:
        ...
        if output_format == 'yaml':
            ...
            click.echo(yaml.safe_dump(settings, default_flow_style=False, sort_keys=False))
        else:
            # Table format
            click.echo(f"Configuration file: {config_path}")
            click.echo()
            ...
            click.echo(f"  Large file threshold: {_format_bytes(settings.get('large_file_threshold', 0))} ({settings.get('large_file_threshold', 'N/A')} bytes)")
            click.echo(f"  Default output format: {settings.get('default_output_format', 'N/A')}")
            ...
            click.echo(f"  Total files processed: {settings.get('total_files_processed', 0)}")
            click.echo(f"  Total duplicates found: {settings.get('total_duplicates_found', 0)}")
...
@config.command()
@click.option('--limit', type=int, default=10, help='Maximum number of entries to show (default: 10)')
...
def _convert_value(key: str, value: str) -> Any:
    ...
    if key == 'fuzzy_threshold':
        ...
        if value.lower() in ['true', '1', 'yes', 'on']:
            ...
            raise ValueError(f"{key} must be a boolean value (true/false, yes/no, 1/0, on/off)")
