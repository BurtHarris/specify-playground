...
def show_auto_decision_concise(files):
    ...
    for i, file_info in enumerate(files):
        ...
        descriptive_words = ['full', 'video', 'hd', 'compilation', 'part', 'scene', 'episode']
...
def get_user_choice_simple(files):
    ...
    while True:
        choice = input("Accept (Y), Keep specific # (k3), Delete specific # (d2), Skip (s), Quit (q): ").strip().lower()
...
def show_auto_decision(files):
    ...
    for rank, (score, i, filename, reasons) in enumerate(scores, 1):
        ...
        print(f"      Score: {score:+3d} - {', '.join(reasons) if reasons else 'no bonuses/penalties'}")
        print()
...
def main():
    ...
    for group_num, group in enumerate(duplicate_groups, 1):
        ...
        if all_numbered or (has_base_file and any(is_numbered_duplicate(f['path']) for f in files)):
            ...
            if has_base_file:
                ...
                files_to_delete = [i + 1 for i, f in enumerate(files) if is_numbered_duplicate(f['path'])]
            else:
                ...
                sorted_indices = sorted(range(len(files)), key=lambda i: get_number(files[i]['path']))
                files_to_delete = [i + 1 for i in sorted_indices[1:]]  # Keep first, delete rest
        ...
        if action == 'quit':
            ...
            files_to_delete = [i + 1 for i in range(len(files)) if (i + 1) not in files_to_delete]
            action = 'delete'
