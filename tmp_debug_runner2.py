...
with patch('pathlib.Path.exists', return_value=True), \
     ...
     patch('pathlib.Path.iterdir', return_value=mock_iter), \
     patch('os.access', return_value=True):
    entries = list(Path('/test').iterdir())
    print('Entries returned by iterdir():', entries)
    for e in entries:
        try:
            print('Entry is_file():', e.is_file(), 'suffix:', getattr(e, 'suffix', None), 'is_video_ext:', getattr(e, 'suffix', '').lower() in {'.mp4', '.mkv', '.mov'})
        except Exception as ex:
            print('Entry inspect error:', ex)
    # Direct validations
    for e in entries:
        try:
            print('Direct validate_file call for', e, '->', scanner.validate_file(e))
        except Exception as ex:
