...
class TestFuzzyMatchingIntegration:
    ...
    def create_test_videos(self):
        ...
        series_episodes = [
            ...
            (
                ...
                b"GoT Episode 1 alt content" * 1000,
            ),
            (
                "Game_of_Thrones_Season1_Episode1.mov",
                b"GoT Episode 1 long content" * 1000,
            ),
        ]
    ...
    def test_end_to_end_fuzzy_matching_high_similarity(self):
        ...
        potential_matches = self.detector.find_potential_matches(
            scanned_files, threshold=0.8
        )
                ...
                if len(set(extensions)) > 1:  # Different extensions
                    ...
                    for stem in stems[1:]:
                        ...
                        if (
                            ...
                            or compare_stem in base_stem
                        ):
                            extension_match_found = True
                            break

        assert (
            extension_match_found
        ), "Should match files with same name but different extensions"
    ...
    def test_fuzzy_matching_threshold_filtering(self):
        ...
        high_threshold_matches = self.detector.find_potential_matches(
            scanned_files, threshold=0.9
        )

        # Test with medium threshold
        medium_threshold_matches = self.detector.find_potential_matches(
            scanned_files, threshold=0.7
        )

        # Test with low threshold (permissive matching)
        low_threshold_matches = self.detector.find_potential_matches(
            scanned_files, threshold=0.5
        )
    ...
    def test_fuzzy_matching_punctuation_normalization(self):
        ...
        assert (
            punctuation_match_found
        ), "Should match files despite punctuation differences"
    ...
    def test_fuzzy_matching_recognizes_tv_series_patterns(self):
        ...
        assert (
            episode_match_found
        ), "Should match TV series episodes despite naming differences"
        ...
        for group in potential_matches:
            if len(group.files) >= 2:
                ...
                for file in group.files[1:]:
                    ...
                    assert (
                        len(common_chars) >= 3
                    ), f"Files should have some similarity: {first_name} vs {other_name}"
    ...
    def test_fuzzy_matching_performance_with_many_files(self):
        ...
        ), "Should find some potential matches in large dataset"
