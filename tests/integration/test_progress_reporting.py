...
class TestProgressReportingIntegration:
    ...
    def test_progress_reporting_during_cli_scan(self):
        ...
        result = self.cli_runner.invoke(
            main, ["--progress", "--verbose", str(self.temp_dir)]
        )
    ...
    def test_progress_reporting_current_file_display(self):
        ...
        mentions_processing = any(
            indicator in output.lower() for indicator in processing_indicators
        )

        assert (
            mentions_files or mentions_processing
        ), "Should show current file or processing status"
    ...
    def test_progress_reporting_no_progress_option(self):
        ...
        assert (
            not has_progress or len(output) < 500
        ), "Should not show progress with --no-progress"
    ...
    def test_progress_reporting_recursive_scan_indication(self):
        ...
        recursive_result = self.cli_runner.invoke(
            main, ["--recursive", "--progress", str(self.temp_dir)]
        )

        # Integration test: Non-recursive scan progress
        non_recursive_result = self.cli_runner.invoke(
            main, ["--no-recursive", "--progress", str(self.temp_dir)]
        )
    ...
    def test_progress_reporting_handles_errors_gracefully(self):
        ...
        try:
            ...
            has_error_handling = any(
                indicator in output for indicator in error_indicators
            )
            ...
            test_passes = (has_error_handling and has_progress) or (
                has_progress and result.exit_code == 0
            )
            ...
            assert (
                test_passes
            ), "Should handle errors gracefully while maintaining progress"
        ...
        try:
            scanned_files = list(
                self.scanner.scan_directory(
                    Path(self.temp_dir), progress_reporter=mock_reporter
                )
            )
            ...
            ), "Should generate progress events during scanning"
