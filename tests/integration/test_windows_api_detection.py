...
class TestWindowsAPIDetectionIntegration:
    ...
    @pytest.mark.skipif(not is_windows_platform(), reason="Windows API tests require Windows")
    ...
    @pytest.mark.skipif(is_windows_platform(), reason="Non-Windows API tests require non-Windows")
    def test_non_windows_platform_raises_error(self, tmp_path):
        """Test that non-Windows platforms raise appropriate errors."""
        test_file = tmp_path / "test_video.mp4"
        test_file.write_bytes(b"fake video content")
        
        with pytest.raises(RuntimeError, match="OneDrive detection only supported on Windows"):
    ...
    def test_cloud_only_file_detection(self):
        ...
        with patch('ctypes.windll.kernel32.GetFileAttributesW') as mock_get_attrs:
            with patch.object(Path, 'exists', return_value=True):
                ...
                mock_get_attrs.return_value = 0x00400000  # FILE_ATTRIBUTE_RECALL_ON_DATA_ACCESS
